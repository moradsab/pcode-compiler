...
%left         HYPERUNARY
%left  <node> ARROW DOT LPAREN LBRCKT FOPT
...
%type  <node> iter_stemnt switch_stemnt break_stemnt continue_stemnt fopt_stemnt
%type  <node> return_stemnt goto_stemnt null_stemnt
%type  <node> do_stemnt while_stemnt for_stemnt parfor_stemnt
...
stemnt:  expr_stemnt
      ...
      |  fopt_stemnt     //YOSI triggers flat tree 
        {
            treenode *tmp_node = make_node(TN_STEMNT, ParseStack->contxt);
            tmp_node->rnode = $1;
            $$ = tmp_node;
...
fopt_stemnt: FOPT SEMICOLON   //YOSI  FOPT triggers flat tree
        {
            $1->hdr.type = TN_JUMP;
            free_tree($2);
        }
...
parfor_stemnt: PARFOR LPAREN opt_expr SEMICOLON
                        opt_expr SEMICOLON opt_expr RPAREN stemnt
        {
            $1->hdr.type = TN_FOR;
            $1->init = $3;
